...
screen = pygame.display.set_mode([screen_w, screen_h], flags=pygame.HIDDEN)
...
# writer = cv2.VideoWriter('videos/result3.mp4', cv2.VideoWriter_fourcc(*'H264'), 20, (1920,1080))
...
bass = {"start": 50, "stop": 100, "count": 4}
heavy_area = {"start": 120, "stop": 250, "count": 12}
low_mids = {"start": 251, "stop": 2000, "count": 18}
high_mids = {"start": 2001, "stop": 6000, "count": 8}
...
for g in tmp_bars:
    ...
    for c in g:
        gr.append(
            RotatedAverageAudioBar(circleX+radius*math.cos(math.radians(ang - 90)), circleY+radius*math.sin(math.radians(ang - 90)), c, (255, 0, 255), angle=ang, width=2, min_height=10, max_height=50))
        ang += angle_dt
...
while running:
    ...
    cv2.imshow('Frame',screen_data)
    frames.append(screen_data)
    # writer.write(screen_data)
    pygame.display.flip()

fps = int(len(frames) / song_length)

writer = cv2.VideoWriter('videos/result3.mp4', cv2.VideoWriter_fourcc(*'H264'), fps, (1920,1080))
